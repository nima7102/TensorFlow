...
The lack of eager execution was one of the main complaints against TensorFlow, which is relatable to everyone. The task of executing the whole graph and then trying to debug based on the errors was very tedious. Especially, since values of intermediate results haven't been accessible without printing them out by mixing in debug statements into the production code.
